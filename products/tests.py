...
class TestReviews(APITestCase):
    def setUp(self) -> None:
        ...
        self.payload = {'product': self.product3.id,
                   'text': 'Cool!',
                   'rating': 4
                   }
    ...
    def test_create_review_with_wrong_rating_400(self):
        '''Тест проверяет создание отзыва с рейтингом который выше 5.  Ожидаемый статус: 400'''
        data = self.payload.copy()
        data['rating'] = 100
        client = self.client_class()
        client.credentials(HTTP_AUTHORIZATION=f'Token {self.user_token.key}')
    ...
    def test_create_review_with_normal_rating_400(self):
        '''Тест проверяет создание отзыва с рейтингом который выше 5.  Ожидаемый статус: 400'''
        data = self.payload.copy()
        data['rating'] = 4
        client = self.client_class()
...
#TODO: pytest  (отзывы)
#TODO Точка breakpoint
#TODO фабрика данных (test factory)
#TODO factorBoy
#TODO Model Bakery , Фикстура
